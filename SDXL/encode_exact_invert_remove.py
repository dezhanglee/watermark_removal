...
from inversion import stable_diffusion_xl_pipe, generate, exact_inversion
from attacks import *
...
device = args.device
n = 4 * 128 * 128  # the length of a PRC codeword
method = args.method
...
def invert(
    ...
    global pipe
    pipe = pipe.to(device)
    start_latents=start_latents.to(device)
    # Encode prompt
    prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds = pipe.to(device).encode_prompt(
        prompt=prompt,
        device=device,
    ...
    # print("text embed", text_embeddings)
    ...
    for i in tqdm(range(1, num_inference_steps), total=num_inference_steps - 1):
        ...
        noise_pred = pipe.unet(
            ...
            t,
            encoder_hidden_states=torch.cat([prompt_embeds] * 2) if do_classifier_free_guidance else prompt_embeds,
...
def sample(
    ...
    if start_latents is None:
        start_latents = torch.randn(
            (num_images_per_prompt, pipe.unet.in_channels, height // 8, width // 8),
            device=device,
    ...
    for i in tqdm(range(start_step, num_inference_steps)):
        ...
        direction_to_xt = (1 - alpha_t_prev).sqrt() * noise_pred
        latents = alpha_t_prev.sqrt() * predicted_x0 + direction_to_xt
    ...
    print(latents.shape)
    with torch.no_grad():
        images = pipe.vae.decode(latents, return_dict=False)[0]  # [B, 3, H, W]
...
pipe = stable_diffusion_xl_pipe(solver_order=1, model_id=model_id, cache_dir=hf_cache_dir)
...
for i in tqdm(range(test_num)):
    ...
    if nowm:
        init_latents_np = np.random.randn(1, 4, 128, 128)
        init_latents = torch.from_numpy(init_latents_np).to(torch.float64).to(device)
    else:
        if method == 'prc':
            prc_codeword = Encode(encoding_key)
            init_latents = prc_gaussians.sample(prc_codeword).reshape(1, 4, 128, 128).to(device)
        elif method == 'gs':
    ...
    orig_image=sample(prompt=current_prompt, start_latents=init_latents, num_inference_steps=args.inf_steps,)[0]
    ...
    if args.inversion == 'exact':
        ...
        # inverted_latents = exact_inversion(orig_image,
        #                                prompt="",
        #                                    num_inference_steps=30,
        #                                test_num_inference_steps=30,
        ...
        # reversed_latents =inverted_latents
    ...
    print(reversed_latents.element_size() * reversed_latents.nelement())
    print(reversed_latents_attack.element_size() * reversed_latents_attack.nelement())
    if args.inversion == 'null':
        with torch.no_grad():
            orig_image=sample(current_prompt, start_latents=reversed_latents_attack,
                              start_step=start_step, 
                              num_inference_steps=args.inf_steps*2,
                              guidance_scale=0.3,
