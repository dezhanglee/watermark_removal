...
#     convert_tensor = tfms.ToTensor()
    ...
    if args.inversion == 'exact':
        ...
        inverted_latents = invert(lat, "", num_inference_steps=args.inf_steps)
...
#                                        )
        ...
        start_step = 30
        inverted_latents = invert(lat, current_prompt, num_inference_steps=args.inf_steps)
        reversed_latents = inverted_latents[-(start_step + 1)][None]
#         reversed_latents = exact_inversion(orig_image,
...
#                                 )
        ...
        orig_image=sample(prompt=current_prompt, start_latents=reversed_latents_attack, start_step=start_step,num_inference_steps=args.inf_steps,
                           )[0]
    else:
...
#         combined_results.append(combined_result)
        print(acc_metric)
    is_removed=not combined_result and ssim_const  > 0.7
